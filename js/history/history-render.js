...
(() => {
  ...
  function renderHistoryDebug(data) {
    ...
    if (!debug) return '';
    return `<section class="panel-card history-debug-card" style="border:2px solid #f97316;background:#fff7ed;"><div class="history-section-head"><div><div class="eyebrow">Temporary Debug</div><h3>History HQ Last 10 Debug</h3></div></div><p class="history-support-copy">Screenshot this panel. Compare Global Log vs After Score Filter vs HQ Momentum.</p><details open><summary><strong>First 10 Global Log</strong></summary>${renderDebugRows(debug.firstTenGlobalLog)}</details><details open><summary><strong>First 10 After Score Filter</strong></summary>${renderDebugRows(debug.firstTenAfterScoreFilter)}</details><details open><summary><strong>HQ Momentum IDs</strong></summary><pre style="white-space:pre-wrap;font-size:11px;background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:8px;">${escapeHtml(JSON.stringify(debug.hqMomentum, null, 2))}</pre></details><details><summary><strong>Season Recent Cards</strong></summary>${renderDebugRows(debug.seasonRecentCards)}</details></section>`;
  }
