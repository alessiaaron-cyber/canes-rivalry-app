...
(() => {
  ...
  function picksFromDisplayedSideContext() { const buckets = {}; const keys = sideKeys(); const displayState = CR.gameDay?.mode === 'pregame' ? { users: CR.gameDay?.pregame || {} } : finalData(); keys.forEach((key, index) => { const side = CR.gameDayRenderUtils?.getSideContext?.(index, displayState) || {}; buckets[key] = (side.picks || []).map((pick) => pick.player || pick.playerName || pick.name || '').filter(Boolean); }); return buckets; }
  function currentPregameForSheet() { const buffered = edit().getBuffer?.(); if (buffered) return buffered; return picksFromDisplayedSideContext(); }
  function debugSheetLine(buckets = {}) { const keys = sideKeys(); return `Debug: ${keys.map((key, index) => `${state().displayName?.(index, source()) || `Player ${index + 1}`} [${pickLabels(buckets?.[key] || []).join(', ') || 'empty'}]`).join(' • ')}`; }
  ...
  function openSheetBuffer() { return currentPregameForSheet(); }
  function setModalOpen(isOpen) { const modal = $('#manageSheet'); if (!modal) return; if (isOpen) { edit().openBuffer?.(openSheetBuffer()); modal.classList.add('is-open'); modal.setAttribute('aria-hidden', 'false'); CR.ui?.lockBodyScroll?.('manage-sheet-open'); renderManageSheet(); } else { edit().clear?.(); modal.classList.remove('is-open'); modal.setAttribute('aria-hidden', 'true'); CR.ui?.unlockBodyScroll?.('manage-sheet-open'); } }
  function sheetHeading() { if (CR.gameDay?.mode === 'live') return { title: 'Manage Live Picks', detail: 'Update picks for the live game currently shown.' }; if (CR.gameDay?.mode === 'final') return { title: 'Manage Final Picks', detail: 'Update picks for the final game currently shown.' }; return { title: 'Manage Pregame Picks', detail: 'Update picks before puck drop.' }; }

  function renderManageSheet() { const actions = $('#manageSheetActions'); const saveButton = $('#saveSheet'); const title = $('#manageSheetTitle'); const copy = document.querySelector('.gd-sheet-copy'); if (!actions) return; const picksEnabled = canManagePicks(); const undoEnabled = canUndoDraftPick(); const bufferPregame = currentPregameForSheet(); const keys = sideKeys(); const selectedPlayers = keys.flatMap((key) => pickLabels(bufferPregame?.[key] || [])); const heading = sheetHeading(); if (title) title.textContent = heading.title; if (copy) copy.textContent = heading.detail; if (saveButton) { saveButton.disabled = !picksEnabled; saveButton.textContent = CR.gameDay?.mode === 'live' ? 'Save Live Picks' : CR.gameDay?.mode === 'final' ? 'Save Final Picks' : 'Save Picks'; } const undoHtml = undoEnabled ? '<button class="cr-button secondary gd-inline-action" id="undoDraftPick" type="button">Undo Last Draft Pick</button>' : ''; const debugHtml = `<div class="gd-sheet-pick"><small>${debugSheetLine(bufferPregame)}</small></div>`; const controlsHtml = keys.flatMap((sideKey, sideIndex) => [0, 1].map((index) => { const selected = pickLabel(bufferPregame?.[sideKey]?.[index] || ''); const options = [''].concat(roster().map((player) => player.name)).map((name) => { const disabled = !picksEnabled || (name && selectedPlayers.includes(name) && name !== selected); return `<option value="${name}" ${name === selected ? 'selected' : ''} ${disabled ? 'disabled' : ''}>${name || 'Open slot'}</option>`; }).join(''); return `<div class="gd-sheet-pick ${!picksEnabled ? 'is-disabled' : ''}"><strong>${state().displayName?.(sideIndex, source()) || `Player ${sideIndex + 1}`} Pick ${index + 1}</strong><small>${picksEnabled ? 'Override displayed player' : 'Locked'}</small><select class="gd-sheet-select" data-side-key="${sideKey}" data-index="${index}" ${picksEnabled ? '' : 'disabled'}>${options}</select></div>`; }).join('')).join(''); actions.innerHTML = undoHtml + debugHtml + controlsHtml; $('#undoDraftPick')?.addEventListener('click', undoLastDraftPick); actions.querySelectorAll('.gd-sheet-select').forEach((select) => { select.addEventListener('change', (event) => { if (!canManagePicks()) return; edit().updatePick?.(event.target.dataset.sideKey, Number(event.target.dataset.index), event.target.value); renderManageSheet(); }); }); }
  async function saveGameDayPicks() { const button = $('#saveSheet'); try { if (!canManagePicks()) throw new Error('Picks cannot be saved right now.'); CR.ui?.setActionBusy?.(button, true, { label: 'Saving…' }); const nextPregame = clone(edit().getBuffer?.() || currentPregameForSheet() || {}); const nextDraft = draft().computeDraftState?.(nextPregame, CR.gameDay?.users || [], CR.gameDay?.draft || {}) || CR.gameDay?.draft; CR.gameDay.pregame = nextPregame; CR.gameDay.draft = nextDraft; await CR.gameDaySaveService?.savePregamePicks?.(CR.gameDay.currentGameId, nextPregame, nextDraft); edit().clear?.(); setModalOpen(false); await refreshGameDayData({ flash: true }); CR.showToast?.('Picks saved'); } catch (error) { console.error('Game Day pick save failed', error); setModalOpen(false); CR.showToast?.({ message: error?.message || 'Could not save picks', tier: 'warning' }); } finally { CR.ui?.setActionBusy?.(button, false); } }
  async function undoLastDraftPick() { const button = $('#undoDraftPick'); try { if (!canUndoDraftPick()) throw new Error('Undo is only available before the game starts.'); CR.ui?.setActionBusy?.(button, true, { label: 'Undoing…' }); await CR.gameDaySaveService?.undoLastDraftPick?.(CR.gameDay.currentGameId); edit().clear?.(); setModalOpen(false); await refreshGameDayData({ flash: true }); CR.showToast?.('Last draft pick undone'); } catch (error) { console.error('Draft undo failed', error); setModalOpen(false); CR.showToast?.({ message: error?.message || 'Could not undo last pick', tier: 'warning' }); } finally { CR.ui?.setActionBusy?.(button, false); } }
