...
(() => {
  ...
  function pregameStructuredFrom(pregame = CR.gameDay?.pregame || {}) { const sideKeys = draft().sideKeys?.(CR.gameDay?.users || []) || state().sideKeys?.(source()) || []; return sideKeys.reduce((acc, key) => { acc[key] = pickLabels(pregame?.[key] || []).map((player) => ({ player })); return acc; }, {}); }
  ...
  function statusCopy(picksEnabled) { if (!hasDisplayedGame()) return { title: 'No displayed game', detail: 'Manage is available once Game Day is showing a game.', saveLabel: 'No Game' }; if (CR.gameDay?.mode === 'live') return { title: 'Manage displayed game picks', detail: 'Changes apply to the live game currently shown here.', saveLabel: picksEnabled ? 'Save Live Picks' : 'Locked' }; if (CR.gameDay?.mode === 'final') return { title: 'Manage displayed final picks', detail: 'Changes apply to the final game currently shown here.', saveLabel: picksEnabled ? 'Save Final Picks' : 'Locked' }; return { title: 'Manage displayed game picks', detail: hasScheduledGame() ? 'Changes apply to the pregame picks currently shown here.' : 'Changes apply to the displayed game.', saveLabel: picksEnabled ? 'Save Picks' : 'Locked' }; }

  function renderManageSheet() { const actions = $('#manageSheetActions'); const saveButton = $('#saveSheet'); if (!actions) return; const picksEnabled = canManagePicks(); const bufferPregame = currentPregameForSheet(); const sideKeys = draft().sideKeys?.(CR.gameDay?.users || []) || state().sideKeys?.(source()) || []; const selectedPlayers = sideKeys.flatMap((key) => pickLabels(bufferPregame?.[key] || [])); const status = statusCopy(picksEnabled); if (saveButton) { saveButton.disabled = !picksEnabled; saveButton.textContent = status.saveLabel; } const statusHtml = `<div class="gd-sheet-pick ${!picksEnabled ? 'is-disabled' : ''}"><strong>${status.title}</strong><small>${status.detail}</small></div>`; const undoHtml = `<button class="cr-button secondary gd-inline-action" id="undoDraftPick" type="button" ${picksEnabled ? '' : 'disabled'}>Undo Last Draft Pick</button>`; const controlsHtml = sideKeys.flatMap((sideKey, sideIndex) => [0, 1].map((index) => { const selected = pickLabel(bufferPregame?.[sideKey]?.[index] || ''); const options = [''].concat(roster().map((player) => player.name)).map((name) => { const disabled = !picksEnabled || (name && selectedPlayers.includes(name) && name !== selected); return `<option value="${name}" ${name === selected ? 'selected' : ''} ${disabled ? 'disabled' : ''}>${name || 'Open slot'}</option>`; }).join(''); return `<div class="gd-sheet-pick ${!picksEnabled ? 'is-disabled' : ''}"><strong>${state().displayName?.(sideIndex, source()) || `Player ${sideIndex + 1}`} Pick ${index + 1}</strong><small>${picksEnabled ? (CR.gameDay?.mode === 'live' ? 'Swap picked player only' : 'Override displayed player') : 'Locked'}</small><select class="gd-sheet-select" data-side-key="${sideKey}" data-index="${index}" ${picksEnabled ? '' : 'disabled'}>${options}</select></div>`; }).join('')).join(''); actions.innerHTML = statusHtml + undoHtml + controlsHtml; $('#undoDraftPick')?.addEventListener('click', undoLastDraftPick); actions.querySelectorAll('.gd-sheet-select').forEach((select) => { select.addEventListener('change', (event) => { if (!canManagePicks()) return; edit().updatePick?.(event.target.dataset.sideKey, Number(event.target.dataset.index), event.target.value); renderManageSheet(); }); }); }
