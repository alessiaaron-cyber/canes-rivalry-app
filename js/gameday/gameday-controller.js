...
(() => {
  ...
  function currentPregameForSheet() { return edit().getBuffer?.() || CR.gameDay?.pregame || {}; }

  function pregameStructuredFrom(pregame = CR.gameDay?.pregame || {}) {
    const sideKeys = draft().sideKeys?.(CR.gameDay?.users || []) || state().sideKeys?.(source()) || [];
    return sideKeys.reduce((acc, key) => {
      acc[key] = pickLabels(pregame?.[key] || []).map((player) => ({ player }));
      return acc;
    }, {});
  }
  ...
  function applyGameDayData(data = {}) {
    ...
    ensureState();
    CR.gameDay = { ...CR.gameDay, ...data, game: { ...(CR.gameDay.game || {}), ...(data.game || {}) }, carryover: { ...(CR.gameDay.carryover || {}), ...(data.carryover || {}) }, draft: { ...(CR.gameDay.draft || {}), ...(data.draft || {}) }, live: { ...(CR.gameDay.live || {}), ...(data.live || {}), scores: { ...(CR.gameDay.live?.scores || {}), ...(data.live?.scores || {}) }, users: data.live?.users || CR.gameDay.live?.users || {} }, pregame: data.pregame || CR.gameDay.pregame || {}, roster: data.roster || CR.gameDay.roster || [] };
    CR.gameDayRoster = CR.gameDay.roster || CR.gameDayRoster || [];
    CR.identity?.applyUserColorVariables?.({ users: CR.gameDay.users });
    return CR.gameDay;
  }

  async function refreshGameDayData(options = {}) { if (options.skipIfEditing && isEditing()) return CR.gameDay; try { const data = await CR.gameDayDataService?.fetchGameDayData?.(); if (data) applyGameDayData(data); edit().clear?.(); renderGameDayState(data?.mode || CR.gameDay?.mode || 'pregame'); if (options.flash) CR.flashSync?.(); if (options.toast) CR.showToast?.('Game Day updated'); return CR.gameDay; } catch (error) { console.error('Game Day refresh failed', error); if (options.toast) CR.showToast?.({ message: 'Could not refresh Game Day', tier: 'warning' }); return CR.gameDay; } }

  function setModalOpen(isOpen) { const modal = $('#manageSheet'); if (!modal) return; if (isOpen) { edit().openBuffer?.(CR.gameDay?.pregame || {}); modal.classList.add('is-open'); CR.ui?.lockBodyScroll?.('manage-sheet-open'); renderManageSheet(); } else { edit().clear?.(); modal.classList.remove('is-open'); CR.ui?.unlockBodyScroll?.('manage-sheet-open'); } }
  function statusCopy(picksEnabled) { if (!hasScheduledGame()) return { title: 'Schedule pending', detail: 'Picks can be managed after the next game is scheduled.', saveLabel: 'Schedule Pending' }; if (CR.gameDay?.mode === 'live') return { title: 'Live game pick management', detail: 'Pick swaps are allowed here; live stats remain read-only and come from NHL sync.', saveLabel: picksEnabled ? 'Save Live Picks' : 'Locked' }; if (CR.gameDay?.mode === 'final') return { title: 'Final game locked', detail: 'Finalized games are read-only on Game Day. Use History for stat corrections.', saveLabel: 'Final Locked' }; return { title: 'Admin override', detail: 'Use this only to fix mistakes or draft for someone unavailable.', saveLabel: picksEnabled ? 'Save Override' : 'Locked' }; }

  function renderManageSheet() {
    const actions = $('#manageSheetActions'); const saveButton = $('#saveSheet'); if (!actions) return;
    const picksEnabled = canManagePicks(); const bufferPregame = currentPregameForSheet(); const sideKeys = draft().sideKeys?.(CR.gameDay?.users || []) || state().sideKeys?.(source()) || []; const selectedPlayers = sideKeys.flatMap((key) => pickLabels(bufferPregame?.[key] || [])); const status = statusCopy(picksEnabled);
    ...
    const controlsHtml = sideKeys.flatMap((sideKey, sideIndex) => [0, 1].map((index) => { const selected = pickLabel(bufferPregame?.[sideKey]?.[index] || ''); const options = [''].concat(roster().map((player) => player.name)).map((name) => { const disabled = !picksEnabled || (name && selectedPlayers.includes(name) && name !== selected); return `<option value="${name}" ${name === selected ? 'selected' : ''} ${disabled ? 'disabled' : ''}>${name || 'Open slot'}</option>`; }).join(''); return `<div class="gd-sheet-pick ${!picksEnabled ? 'is-disabled' : ''}"><strong>${state().displayName?.(sideIndex, source()) || `Player ${sideIndex + 1}`} Pick ${index + 1}</strong><small>${picksEnabled ? (CR.gameDay?.mode === 'live' ? 'Swap picked player only' : 'Override locked player') : 'Locked'}</small><select class="gd-sheet-select" data-side-key="${sideKey}" data-index="${index}" ${picksEnabled ? '' : 'disabled'}>${options}</select></div>`; }).join('')).join('');
    actions.innerHTML = statusHtml + undoHtml + controlsHtml;
    $('#undoDraftPick')?.addEventListener('click', undoLastDraftPick);
    actions.querySelectorAll('.gd-sheet-select').forEach((select) => { select.addEventListener('change', (event) => { if (!canManagePicks()) return; edit().updatePick?.(event.target.dataset.sideKey, Number(event.target.dataset.index), event.target.value); renderManageSheet(); }); });
  }

  async function saveGameDayPicks() { const button = $('#saveSheet'); try { if (!canManagePicks()) throw new Error('Picks cannot be saved right now.'); CR.ui?.setActionBusy?.(button, true, { label: 'Saving…' }); const nextPregame = clone(edit().getBuffer?.() || CR.gameDay?.pregame || {}); const nextDraft = draft().computeDraftState?.(nextPregame, CR.gameDay?.users || [], CR.gameDay?.draft || {}) || CR.gameDay?.draft; CR.gameDay.pregame = nextPregame; CR.gameDay.draft = nextDraft; await CR.gameDaySaveService?.savePregamePicks?.(CR.gameDay.currentGameId, nextPregame, nextDraft); edit().clear?.(); setModalOpen(false); await refreshGameDayData({ flash: true }); CR.showToast?.('Picks saved'); } catch (error) { console.error('Game Day pick save failed', error); CR.showToast?.({ message: error?.message || 'Could not save picks', tier: 'warning' }); } finally { CR.ui?.setActionBusy?.(button, false); } }
  async function undoLastDraftPick() { const button = $('#undoDraftPick'); try { if (!canManagePicks()) throw new Error('Draft undo is not available right now.'); CR.ui?.setActionBusy?.(button, true, { label: 'Undoing…' }); await CR.gameDaySaveService?.undoLastDraftPick?.(CR.gameDay.currentGameId); edit().clear?.(); setModalOpen(false); await refreshGameDayData({ flash: true }); CR.showToast?.('Last draft pick undone'); } catch (error) { console.error('Draft undo failed', error); CR.showToast?.({ message: error?.message || 'Could not undo last pick', tier: 'warning' }); } finally { CR.ui?.setActionBusy?.(button, false); } }
  ...
  function renderHero() { return render().renderHeroSection?.({ mode: CR.gameDay.mode, game: CR.gameDay.game, pregameUsers: pregameStructured(), live: CR.gameDay.live, final: finalData(), isPlayoffs: isPlayoffs(), winnerText, nextDraftSide: nextDraftSide(), draft: CR.gameDay.draft }) || ''; }
  ...
  function renderGameDayState(mode = CR.gameDay?.mode || 'pregame') { ensureState(); CR.gameDay.mode = mode; CR.identity?.applyUserColorVariables?.({ users: CR.gameDay.users }); const container = $('#gameDayContent'); const view = $('#gameDayView'); if (!container || !view) return; view.classList.toggle('mode-playoffs', isPlayoffs()); view.classList.toggle('is-realtime-changed', CR.ui?.isChanged?.('gameday:sync')); container.innerHTML = [renderHero(), mode === 'pregame' ? renderPregame() : '', mode === 'live' ? renderLive() : '', mode === 'final' ? renderFinal() : ''].join(''); const stateTitle = $('#stateTitle'); const stateBadge = $('#stateBadge'); if (stateTitle) stateTitle.textContent = modeLabel(mode); if (stateBadge) stateBadge.textContent = isPlayoffs() ? 'Playoffs' : (mode === 'pregame' ? 'Regular' : modeLabel(mode)); updateGlobalIndicators(); bindInteractions(); }
  function payloadBelongsToCurrentGame(payload) { const row = payload.new || payload.old || {}; if (!CR.gameDay?.currentGameId) return true; if (payload.table === 'games') return String(row.id || '') === String(CR.gameDay.currentGameId); if (payload.table === 'picks') return String(row.game_id || '') === String(CR.gameDay.currentGameId); return true; }
  function registerRealtime() { if (CR.__gameDayRealtimeRegistered || !CR.realtime?.register) return; CR.__gameDayRealtimeRegistered = true; CR.realtime.register('gameday', { tables: ['games', 'picks'], debounceMs: 250, onChange: async (payloads = []) => { if (!payloads.some(payloadBelongsToCurrentGame)) return; if (isEditing()) return; await refreshGameDayData({ flash: true, skipIfEditing: true }); } }); CR.realtime.start?.(); }
  function bindInteractions() { CR.gameDayEvents?.bind?.({ claimedOwner, draftOrder: state().draftOrder?.(source()) || [], nextDraftSide, renderManageSheet, setModalOpen, rerender: renderGameDayState }); }
  function initGameDay() { ensureState(); $('#refreshButton')?.addEventListener('click', () => refreshGameDayData({ toast: true, flash: true })); $('#closeSheet')?.addEventListener('click', () => setModalOpen(false)); $('#saveSheet')?.addEventListener('click', saveGameDayPicks); $('#manageSheet')?.addEventListener('click', (event) => { if (event.target.id === 'manageSheet') setModalOpen(false); }); CR.identity?.applyUserColorVariables?.({ users: CR.gameDay.users }); renderGameDayState(CR.gameDay.mode || 'pregame'); refreshGameDayData(); registerRealtime(); }
