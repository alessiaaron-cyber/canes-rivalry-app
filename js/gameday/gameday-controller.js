...
(() => {
  ...
  function renderManageSheet() { const actions = $('#manageSheetActions'); const saveButton = $('#saveSheet'); const title = $('#manageSheetTitle'); const copy = document.querySelector('.gd-sheet-copy'); if (!actions) return; const picksEnabled = canManagePicks(); const undoEnabled = canUndoDraftPick(); const bufferPregame = currentPregameForSheet(); const keys = sideKeys(); const selectedPlayers = keys.flatMap((key) => pickLabels(bufferPregame?.[key] || [])); const heading = sheetHeading(); if (title) title.textContent = heading.title; if (copy) copy.textContent = heading.detail; if (saveButton) { saveButton.disabled = !picksEnabled; saveButton.textContent = CR.gameDay?.mode === 'live' ? 'Save Live Picks' : CR.gameDay?.mode === 'final' ? 'Save Final Picks' : 'Save Picks'; } const undoHtml = `<button class="cr-button secondary gd-inline-action" id="undoDraftPick" type="button" ${undoEnabled ? '' : 'disabled'}>Undo Last Draft Pick</button>`; const controlsHtml = keys.flatMap((sideKey, sideIndex) => [0, 1].map((index) => { const selected = pickLabel(bufferPregame?.[sideKey]?.[index] || ''); const options = [''].concat(roster().map((player) => player.name)).map((name) => { const disabled = !picksEnabled || (name && selectedPlayers.includes(name) && name !== selected); return `<option value="${name}" ${name === selected ? 'selected' : ''} ${disabled ? 'disabled' : ''}>${name || 'Open slot'}</option>`; }).join(''); return `<div class="gd-sheet-pick ${!picksEnabled ? 'is-disabled' : ''}"><strong>${state().displayName?.(sideIndex, source()) || `Player ${sideIndex + 1}`} Pick ${index + 1}</strong><small>${picksEnabled ? 'Override displayed player' : 'Locked'}</small><select class="gd-sheet-select" data-side-key="${sideKey}" data-index="${index}" ${picksEnabled ? '' : 'disabled'}>${options}</select></div>`; }).join('')).join(''); actions.innerHTML = undoHtml + controlsHtml; $('#undoDraftPick')?.addEventListener('click', undoLastDraftPick); actions.querySelectorAll('.gd-sheet-select').forEach((select) => { select.addEventListener('change', (event) => { if (!canManagePicks()) return; edit().updatePick?.(event.target.dataset.sideKey, Number(event.target.dataset.index), event.target.value); renderManageSheet(); }); }); }
