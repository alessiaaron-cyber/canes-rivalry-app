...
(() => {
  ...
  function renderNextUpTicker(nextGame, isPlayoffs) {
    const label = nextGame?.compactInfo || '';
    if (!label) return '';
    return `<div class="gd-next-up-strip ${isPlayoffs ? 'gd-next-up-strip-playoff' : ''}"><span class="gd-next-up-kicker">NEXT UP</span><span aria-hidden="true">•</span><span>${label}</span></div>`;
  }
