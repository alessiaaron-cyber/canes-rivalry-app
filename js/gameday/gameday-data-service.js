...
(() => {
  ...
  function selectGameForGameDay(games = []) {
    const visibleGames = sortedVisibleGames(games); const now = Date.now();
    ...
    const upcomingGames = visibleGames.filter((game) => !isFinalGame(game) && gameTimestamp(game) >= now).sort((a, b) => gameTimestamp(a) - gameTimestamp(b));
    const latestFinal = visibleGames.filter(isFinalGame).sort((a, b) => gameTimestamp(b) - gameTimestamp(a))[0] || null;
    const nextGame = upcomingGames[0] || null;
    if (latestFinal && nextGame && !isGameDay(nextGame)) return latestFinal;
  ...
  function normalizeGameDayState({ game, nextGame, picks = [], roster = [], profiles = [], gameUserScores = [] }) {
    const users = profiles; const normalizedPicks = picks.map((pick) => normalizePick(pick, roster, profiles)); const pregame = emptyBuckets(users, () => []);
  ...
  async function fetchGameDayData() {
    const db = await CR.getSupabase(); const selection = await fetchGameSelection(db); const game = selection.game; const nextGame = selection.nextGame; const playersPromise = db.from('players').select('*').eq('is_active', true).order('player_name'); const profilesPromise = safeLoadProfiles(db); const picksPromise = game?.id ? db.from('picks').select('*').eq('game_id', game.id).order('pick_slot') : Promise.resolve({ data: [], error: null }); const scoresPromise = game?.id ? db.from('game_user_scores').select('game_id, user_id, points').eq('game_id', game.id) : Promise.resolve({ data: [], error: null });
    const [playersRes, profiles, picksRes, scoresRes] = await Promise.all([playersPromise, profilesPromise, picksPromise, scoresPromise]); if (playersRes.error) throw playersRes.error; if (picksRes.error) throw picksRes.error; if (scoresRes.error) throw scoresRes.error;
    ...
    return normalizeGameDayState({ game, nextGame, picks: picksRes.data || [], roster, profiles, gameUserScores });
  }
