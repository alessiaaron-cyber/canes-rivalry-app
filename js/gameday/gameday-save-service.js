...
(() => {
  ...
  function rowForSlot(gameId, ownerProfile, pickSlot, playerName = '', index = 0) { const cleanPlayerName = pickLabel(playerName); return { game_id: gameId, owner: ownerValue(ownerProfile, index), owner_user_id: ownerProfile?.id || null, pick_slot: pickSlot, player_name: cleanPlayerName, original_pick_text: cleanPlayerName || null, goals: 0, assists: 0, points: 0, picked_by_user_id: cleanPlayerName ? (currentUserId() || null) : null, updated_by_user_id: currentUserId() || null, updated_at: new Date().toISOString() }; }
