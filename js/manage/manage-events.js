...
(() => {
  ...
  function otherUser(user, current = state()) { return (current?.users || []).find((candidate) => String(candidate.id || '') !== String(user?.id || '')) || user || null; }
  ...
  function latestScheduleGame(current = state()) { return (current?.schedule || []).slice().filter((game) => String(game.status || '').toLowerCase() !== 'hidden').sort((a, b) => Number(b.gameNumber || 0) - Number(a.gameNumber || 0))[0] || null; }
  function defaultManualGameType(current = state()) { const latestGame = latestScheduleGame(current); return String(latestGame?.type || '').toLowerCase().includes('playoff') ? 'Playoffs' : 'Regular Season'; }
  function nextManualFirstPicker(current = state()) { const seasonStarter = userById(current?.season?.firstPickerUserId, current) || current?.users?.[0] || null; const previousGame = latestScheduleGame(current); if (!previousGame) { return seasonStarter; } const previousPicker = userById(previousGame.firstPickerUserId, current) || seasonStarter; if (previousGame.usedCarryForward === true) { return previousPicker; } return otherUser(previousPicker, current) || seasonStarter; }
  function nextSeasonFirstPicker(current = state()) { const currentStarter = userById(current?.season?.firstPickerUserId, current) || current?.users?.[0] || null; return otherUser(currentStarter, current) || currentStarter; }
  function resetScheduleDraft() { const current = state(); const nextPicker = nextManualFirstPicker(current); current.scheduleDraft = { date: '', opponent: '', type: defaultManualGameType(current), firstPicker: nextPicker ? userDisplayName(nextPicker) : current.season.firstPicker, firstPickerUserId: nextPicker?.id || current.season.firstPickerUserId || '' }; current.editingScheduleGameId = null; }
  function isScoringProfileLocked(profile, season = state()?.season) { const normalized = String(profile || '').toLowerCase(); if (normalized.includes('playoff')) return season?.playoffScoringLocked === true; return season?.regularScoringLocked === true; }
  ...
  async function saveScoringRules(button) { const current = state(); const profile = current.scoringEditProfile || current.season.scoringProfile || 'Regular'; if (isScoringProfileLocked(profile, current.season)) { CR.showToast?.({ message: `${profile} scoring is locked`, tier: 'warning' }); return; } try { CR.ui?.setActionBusy?.(button, true, { label: 'Saving…' }); await CR.manageDataService.saveScoringRules(current.season); current.scoringEditOpen = false; rerender(); await refreshManageAndGameDay(); CR.showToast?.({ message: 'Scoring rules saved' }); } catch (error) { console.error('Scoring save failed', error); CR.showToast?.({ message: error?.message || 'Could not save scoring rules', tier: 'warning' }); } finally { CR.ui?.setActionBusy?.(button, false); } }
  ...
  function bindManageEvents() {
    ...
    root.addEventListener('click', async (event) => {
      ...
      const startSeason = event.target.closest('[data-manage-start-season]'); if (startSeason) { closeAllSheets(); const nextPicker = nextSeasonFirstPicker(current); current.newSeasonDraft = { seasonLabel: current.newSeasonDraft?.seasonLabel || current.newSeasonOptions?.[0] || '', firstPicker: nextPicker ? userDisplayName(nextPicker) : current.season.firstPicker, firstPickerUserId: nextPicker?.id || current.season.firstPickerUserId || '' }; current.startSeasonOpen = true; rerender(); return; }
      ...
      const editScoring = event.target.closest('[data-manage-edit-scoring]'); if (editScoring) { const profile = editScoring.dataset.manageEditScoring || current.season.scoringProfile || 'Regular'; if (isScoringProfileLocked(profile, current.season)) { CR.showToast?.({ message: `${profile} scoring is locked`, tier: 'warning' }); return; } closeAllSheets(); current.scoringEditProfile = profile; current.scoringEditOpen = true; rerender(); return; }
