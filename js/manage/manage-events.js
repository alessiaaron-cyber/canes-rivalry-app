...
(() => {
  ...
  function resetScheduleDraft() { const current = state(); const nextPicker = nextManualFirstPicker(current); current.scheduleDraft = { date: '', opponent: '', type: 'Regular Season', firstPicker: nextPicker ? userDisplayName(nextPicker) : current.season.firstPicker, firstPickerUserId: nextPicker?.id || current.season.firstPickerUserId || '' }; current.editingScheduleGameId = null; }
